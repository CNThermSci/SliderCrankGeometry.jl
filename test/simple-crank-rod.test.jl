...
@testset "simple-crank-rod.test.jl: kwargs external constructor                   " begin
    ...
    suffKwargsTest(  R=1, L=2,            D=1, Vmax=(10/9)*(π/2))
    suffKwargsTest(  R=1, L=2,            D=1,  Vmin=(1/9)*(π/2))
    suffKwargsTest(  R=1, L=2,      Vdu=(π/2),              r=10)
    suffKwargsTest(  R=1, L=2,      Vdu=(π/2), Vmax=(10/9)*(π/2))
    suffKwargsTest(  R=1, L=2,      Vdu=(π/2),  Vmin=(1/9)*(π/2))
    suffKwargsTest(  R=1, L=2,      Vdu=(π/2),            x0=2/9)
    suffKwargsTest(  S=2, L=2,            D=1,           x0=4e-2)
    suffKwargsTest(  S=2, L=2,            D=1, Vmax=(10/9)*(π/2))
    suffKwargsTest(  S=2, L=2,            D=1,  Vmin=(1/9)*(π/2))
    suffKwargsTest(  S=2, L=2,      Vdu=(π/2),              r=10)
    ...
    suffKwargsTest(rSD=2, L=2,  Vd=(π/1), z=2,              r=10)
    suffKwargsTest(rSD=2, L=2,  Vd=(π/1), z=2, Vmax=(10/9)*(π/2))
    suffKwargsTest(rSD=2, L=2,  Vd=(π/1), z=2,  Vmin=(1/9)*(π/2))
    suffKwargsTest(rSD=2, L=2,  Vd=(π/1), z=2,            x0=2/9)
end
